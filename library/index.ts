export namespace mv
{
    ...
    export const min = <T extends Validator<number>>(validator: T, min: number) =>
        createValidator<mv.infer<T>>((value: unknown) =>
        {
            validator(value)
            if (value < min) throw new TypeError(`Expected ${min} or more, got ${value}`)
        })
    export const max = <T extends Validator<number>>(validator: T, max: number) =>
        createValidator<mv.infer<T>>((value: unknown) =>
        {
            validator(value)
            if (value > max) throw new TypeError(`Expected ${max} or less, got ${value}`)
        })
    export const range = <T extends Validator<number>>(validator: T, min: number, max: number) =>
        createValidator<mv.infer<T>>((value: unknown) =>
        {
            validator(value)
            if (value < min) throw new TypeError(`Expected ${min} or more, got ${value}`)
