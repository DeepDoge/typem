...
export const $unknown = $validator((value: unknown): asserts value is unknown => { })
export const $any = $validator((value: unknown): asserts value is any => { })
...
export const $enum = $validator(<T extends string | number>(value: unknown, validator: Validator<T>, ...values: T[]): asserts value is T =>
{
    validator.assert(value)
    if (!values.includes(value)) throw new TypeError(`Expected ${value} to be one of ${values}`)
})
